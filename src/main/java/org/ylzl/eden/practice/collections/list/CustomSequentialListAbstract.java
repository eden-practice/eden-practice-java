...
public abstract class CustomSequentialListAbstract<E> extends CustomListAbstract<E> {
	...
	public E set(int index, E element) {
		try {
			CustomListIterator<E> e = listIterator(index);
			E oldVal = e.next();
			e.set(element);
			return oldVal;
		} catch (NoSuchElementException exc) {
	...
	public boolean addAll(int index, CustomCollection<? extends E> c) {
		try {
			...
			CustomListIterator<E> e1 = listIterator(index);
			CustomIterator<? extends E> e2 = c.iterator();
			while (e2.hasNext()) {
				e1.add(e2.next());
				modified = true;
			}
	...
	public abstract CustomListIterator<E> listIterator(int index);
}
